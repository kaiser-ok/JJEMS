...
**部署彈性 — 三種拓撲**：同一份 codebase 與 schema，依需求切換為  
**A. 邊緣單站**（大部分案場，不上雲）/ **B. 純 IP 多站**（VPP / 全 IP 化）/ **C. 完整三層**（大型 + 周邊整合）。  
北向統一遵循 [`海辰数能EMS对接数能云MQTT协议规范 V1.4`](#)。詳細見 [`db/DEPLOYMENT.md`](./db/DEPLOYMENT.md)。
